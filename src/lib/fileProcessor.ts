...
function extractBasicExif(_buffer: ArrayBuffer, ext: string): Record<string, string> {
  const data: Record<string, string> = {};
  void 0; // DataView reserved for format detection
...
async function process3DModel(file: File, ext: string, start: number): Promise<ProcessingResult> {
  ...
  if (ext === 'obj') {
    ...
    if (isBinary) {
      ...
      metadata = { ...metadata, triangles: triCount, format: 'Binary STL' } as Record<string, string | number | boolean>;
    } else {
...
async function processDatabase(file: File, ext: string, start: number): Promise<ProcessingResult> {
  ...
  if (ext === 'sql') {
    const content = await file.text();
  void content.split('\n'); // basic line count — full parse not needed here
